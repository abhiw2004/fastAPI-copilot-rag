...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; KnowledgeCopilotBot/1.0)"
}

# Live, current pages. Each entry: (url, doc_type, last_updated, access_level)
# Verified against FastAPI's actual mkdocs.yml nav (docs/en/mkdocs.yml on GitHub).
BASE = "https://fastapi.tiangolo.com"
TODAY = "2024-01-01"
PAGES = [
    # tutorial — onboarding-style pages
    ...
    # advanced — troubleshooting / power-user pages
    ...
    # deployment — closer to "policy"/ops docs
    ...
    # how-to — troubleshooting-flavored
...
# Outdated snapshots for the "stale doc trap" — pulled directly from an old
# git tag of the FastAPI repo instead of Wayback Machine. Every doc page is
# a markdown file at docs/en/docs/<path>.md, and old tags still serve the
# old content via a simple URL, no archive site or manual lookup needed.
# Each entry: (tag, doc_path_under_docs/en/docs/, doc_type, last_updated, access_level)
OLD_PAGES = [
...
def scrape(pages, metadata):
    for url, doc_type, last_updated, access_level in pages:
        ...
        html = fetch_page(url)
        if html is None:
            print(f"  SKIPPED (failed after retries): {url}")
            continue
...
def scrape_old_pages(pages, metadata):
    for tag, doc_path, doc_type, last_updated, access_level in pages:
        ...
        print(f"Fetching old page ({tag}) {url} ...")
